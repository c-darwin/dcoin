...
// MY_PREFIX тут будет соответствовать ID админа ноды
if (!get_community_users($db))
	define('MY_PREFIX', '');
else
	define('MY_PREFIX', $config['pool_admin_user_id'].'_');
...
print_R($row);
	...
	for ($i=0; $i<sizeof($tx_array); $i++) {
		...
		if ($tx_array[$i]['user_id'] === $row['type_id'] &&
			...
			$order_id = $order[1];
			if ($order_id) {
				$sign = hash('sha256', "{$row['amount']}:{$currency_list[$row['currency_id']]}:{$order_id}:{$row['block_id']}:{$tx_id}:{$config['shop_secret_key']}");
				$tx_id = $row['id'];
				$url = $config['shop_callback_url'];
				$ch = curl_init();
